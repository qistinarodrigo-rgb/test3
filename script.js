...
// ========== NOTIFICATION FUNCTION (di ATAS skrin) ==========
function showNotification(message, isSuccess = true) {
    ...
    // Force styles for iPhone
    notif.style.position = 'fixed';
    notif.style.top = navigator.userAgent.includes('iPhone') ? '50px' : '20px';
    notif.style.left = '50%';
    notif.style.transform = 'translateX(-50%)';
    notif.style.backgroundColor = isSuccess ? '#10b981' : '#ef4444';
    ...
    notif.style.border = '1px solid rgba(255,255,255,0.3)';
...
// ========== LOAD ADUAN BY USER EMAIL ==========
async function loadAduanByUser(email) {
    try {
        const snapshot = await db.collection('aduan')
...
function renderAduan(userType) {
    ...
    async function refreshSenarai() {
        ...
        // Admin: lihat semua aduan
        // User: lihat aduan sendiri sahaja
                ...
                try {
                    ...
                    async function saveAduan(gambarData) {
                        const newAduan = {
                            ...
                            userEmail: currentUserEmail, // TAMBAH: simpan email user
                            selesai: false
                        };
                        ...
                        // Notifikasi berjaya di ATAS
                        showNotification('Aduan berjaya dihantar!', true);
                    }
                } catch (error) {
                    console.error('Error:', error);
                    
                    // Notifikasi error di ATAS
